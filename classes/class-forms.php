        ...
        foreach( $this->form_val_arr[ $user_ID ][ $formid ] as $input_name => $input_array ) { 
            ...
            if( isset( $input_array['required']) && $input_array['required'] === true ) {
                ...
                if( $input_array['inputtype'] === 'checkboxes' ) {
                    ...
                    if( $box_checked === 0 ) {
                        $this->UI->create_notice( sprintf( __( 'Oops! You missed something. Please make selection for %s as it is a required option.', 'csv2post' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'wtgcsvexporter' ) );                    
                        return false;                    
                    }                    
                    ...
                    $this->UI->create_notice( sprintf( __( 'Sorry the %s is a required option.', 'csv2post' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'wtgcsvexporter' ) );                    
                    return false;                      
                }
            }           
        }
        ...
        foreach( $_POST as $input_name => $value ) {
            ...
            if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ] ) ) {
                ...
                if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === true ) {
                    ...
                    $this->UI->create_notice( __( 'Possible security violation detected. Please discontinue your action. This event has been logged for investigation.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgcsvexporter' ) );                    
                    return false;    
                }
            }    
        }                  
            ...
            foreach( $this->form_val_arr[ $user_ID ][ $form_id ] as $input_name => $input_array ) {
                ...
                if( !in_array( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['inputtype'], $this->grouped_fields ) ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === false ) {
                        ...
                        if( !isset( $_POST[ $input_name ] ) ) {   
                            $this->UI->create_notice( __( 'Possible security violation. Please discontinue your action. This event has been logged for investigation.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgcsvexporter' ) );                    
                            return false;                        
                        } 
                    }
                }
                ...
                if( $input_array['inputtype'] == 'menu' ) {
                    ...
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'WTG CSV Exporter has security related concerns about a menu on the form you submitted. Please discontinue your action and contact support. This event has been logged for investigation.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgcsvexporter' ) );                    
                        return false;    
                    }
                }                
                ...
                if( $input_array['inputtype'] == 'checkboxes' ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['groupcheckboxes'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['groupcheckboxes'] == false ) {
                        ...
                        if( $failed_to_match === true ) {
                            $this->UI->create_notice( __( 'WTG CSV Exporter security has detected a possible problem with the form you submitted. Please discontinue your action and contact support.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgpf' ) );                    
                            return false;    
                        }
                        ...
                        if( isset( $_POST[ $input_name ] ) && is_array( $_POST[ $input_name ] ) ) {
                            ...
                            // if one or more seletions made but no match this indicates values have been tampered with
                            if( $failed_to_match === true ) {
                                $this->UI->create_notice( __( 'WTG CSV Exporter detected problem relating to the security of the form you submitted. Please discuss this on the WebTechGlobal Forum.', 'csv2post' ), 'error', 'Small', __( 'Something Went Wrong', 'wtgpf' ) );                    
                                return false;    
                            }
                        }
                    }    
                ...
                if( $input_array['inputtype'] == 'radiogroup' ) {
                    ...
                    // if no match, the submitted item value was not registered when the form was parsed
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'WTG CSV Exporter detected a problem and could not continue. Please try again and if it continues contact support.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgcsvexporter' ) );                    
                        return false;    
                    }
                }               
                ...
                if( $input_array['inputtype'] == 'hidden' ) {
                    ...
                    if( !isset( $_POST[ $input_name ] ) ) {
                       // arriving here indicates hidden input missing, a hack has removed it
                        $this->UI->create_notice( __( 'A security matter has arisen. Please discontinue your action and contact support if this message continues to appear. The event has been logged.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgcsvexporter' ) );                    
                        return false;                         
                    } else {
                        // if registered value does not match value submitted, hacker has changed it
                        if( $input_array['currentvalue'] !== $_POST[ $input_name ] ) {
                            $this->UI->create_notice( __( 'WTG CSV Exporter has detected a possible hack attempt. Your activity has been logged. Please discontinue your action and contact support if this message continues to appear without any known reason.', 'csv2post' ), 'warning', 'Small', __( 'Warning', 'wtgcsvexporter' ) );                    
                            return false;                            
                        }
                    }
                }   
